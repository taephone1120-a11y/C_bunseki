...
### 2. 新作の作品紹介文の提案（3選）
分析した人気商品の紹介文の構成をベースに、以下の要素を必ずストーリーの中に自然に組み込んで、そのまま出品に使える紹介文を【3つの異なる雰囲気（例：上品で丁寧なトーン、天然石の物語を伝えるエモーショナルなトーン、日常使いをイメージさせるカジュアルなトーン）】で提案してください。

⚠️【紹介文に必ず入れる必須要素】：
1. **この商品を購入する人が抱えている「悩みや願望」**（例：自分に合うお守りが見つからない、日常にちょっとした特別感が欲しい、指先を美しく見せたい等）
2. **この商品を手に入れることで、お客様がどのような「最高の未来（ベネフィット）」を手に入れられるのか**（例：見るたびに心が癒される、自信を持って身につけられる、周りから褒められる等）
3. **人気商品の紹介文から学んだ「売れる構成とキーワード」**

### 3. 出品時のワンポイントアドバイス
* 発送方法の書き方や、ハッシュタグの選び方など、この紹介文を使って出品する際のアドバイスを1つ教えてください。
"""

    # デザイン枠の中に、完成した紹介文用プロンプトを表示
    st.markdown('<div class="ai-box" style="border-left: 5px solid #4CAF50; background-color: #f9f9f9; padding: 15px; border-radius: 5px;">', unsafe_allow_html=True)
