...
# =================================================================
# 🛍️ 【新機能】商品説明文用のAIプロンプト作成ツール
# =================================================================
st.write("---") # 区切り線
st.header("📝 商品説明文用のAIプロンプト作成ツール")
st.caption("売れ筋商品5選の説明文を分析し、ChatGPTやGeminiに魅力的な説明文を作らせるプロンプトを生成します。")

# 1. 売れ筋5選の商品説明文の入力エリア
st.subheader("1. 分析対象（売れている商品5選の商品説明文）")
st.info("※ライバルや売れ筋の商品から、参考になる説明文を5つコピーして貼り付けてください。")

# 5つの入力欄をタブでスッキリまとめる
tab1, tab2, tab3, tab4, tab5 = st.tabs(["商品①", "商品②", "商品③", "商品④", "商品⑤"])

with tab1:
    desc_1 = st.text_area("1つ目の売れ筋商品説明文", height=150, value="【14kgf】天然石ラピスラズリの一粒リング。\n深い吸い込まれるようなブルーが美しい、最高品質のラピスラズリを使用したシンプルな指輪です。\n12月の誕生石でもあり、お守りやギフトにも最適です。\n■素材：14kgf、天然石ラピスラズリ（約6mm）\n■サイズ：9号/11号/13号")
with tab2:
    desc_2 = st.text_area("2つ目の売れ筋商品説明文", height=150, value="ご覧いただきありがとうございます。\n夜空のようなディープブルーに金色のパイライトが美しく輝くラピスラズリのリングです。\n職人が丁寧にワイヤーでラッピングした、世界に一つだけのハンドメイドジュエリー。\nデイリーユースからオフィス、フォーマルな場面まで幅広くお使いいただけます。\nアレルギー対応のサージカルステンレス製です。")
with tab3:
    desc_3 = st.text_area("3つ目の売れ筋商品説明文", height=150, value="「幸運をもたらす石」として最古の歴史を持つラピスラズリ。\nその原石の魅力をそのまま活かすため、極限までシンプルなワイヤーデザインに仕上げました。\nどの角度から見ても天然石の美しい表情が楽しめます。\n大切な方へのプレゼント用ラッピングも承っております。")
with tab4:
    desc_4 = st.text_area("4つ目の売れ筋商品説明文", height=150, value="天然石の「ありのまま」の美しさを引き出す、華奢なワイヤーアートリング。\n手元を動かすたびに、ラピスラズリの深い青が上品に仕草を彩ります。\n重ね付けにもおすすめのミニマルなデザインです。\n自分へのご褒美にいかがでしょうか。")
with tab5:
    desc_5 = st.text_area("5つ目の売れ筋商品説明文", height=150, value="【新作】大粒ラピスラズリのクラウンベゼルリング。\nアンティーク調の台座に、つやつやの天然石をセッティングしました。\n高級感のあるシルバー925素材（18kgp）を使用しており、長く愛用いただけます。\n無料の簡易ラッピング付きでお届けします。")

# 2. 生成ボタン
desc_generate_btn = st.button("🚀 商品説明文用の命令文（プロンプト）を作成する", type="primary", key="desc_btn")

if desc_generate_btn:
    with st.spinner("📝 商品説明文用のプロンプトを作成中..."):
        
        # 5つの説明文を1つのテキストにまとめる
        analyzed_descriptions = f"""
【売れている商品の説明文①】
{desc_1}
...
"""
        ...
        final_desc_prompt = f"""あなたはハンドメイドマーケット（Creemaやminne）で月商100万円以上を売り上げるトップセラーであり、魅力的な文章で購買意欲をかきたてるコピーライティングのプロフェッショナルです。

以下の【分析対象：売れている商品の商品説明文5選】の構成・キーワード・訴求ポイントを徹底的に分析し、それらの売れる要素を取り入れた、私の新作のための「商品説明文」を新しく作成してください。

---
【分析対象：売れている商品の商品説明文5選】
...
### 1. 売れている商品説明文の分析
* **ヒットを生み出す共通の要素**: 5つの説明文から共通する「購入を後押ししている要素（サイズ・素材の書き方、ブランドストーリー、ギフト訴求、安心感の与え方など）」を分析してください。
* **ターゲットに刺さるキラーフレーズ**: 5つの説明文の中で、特に「おっ」と思わせる魅力的な言葉や表現を3つ抽出して解説してください。

### 2. 新作商品説明文の提案（3パターン）
分析結果を活かし、私の新作の特徴（360度どこからでも美しい、裸のような天然石、はだかのお守りなど）が最も魅力的に伝わる説明文を、以下の3つの異なる切り口で作成してください。

1. **【ストーリー＆情緒重視型】**
   * 石の持つ魅力、作品のコンセプト（はだかのお守り）、作家のこだわりや世界観をエモーショナルに語り、ファンを増やすための文章。
2. **【情報整理＆メリット重視型】**
   * コーディネートの提案（普段使い、オフィス、お守りなど）、商品の仕様（素材・サイズ）、アレルギー対応やギフト対応など、購入者が知りたい情報をスッキリと網羅した、迷わせずに買わせる文章。
3. **【Creema/minneトレンド融合型】**
   * 分析した5つの売れ筋説明文の「良いとこ取り」をし、適度なストーリーと見やすい箇条書きの仕様をバランスよくミックスした、最も手堅く売れる標準的な文章。
"""

    # デザイン枠の中にプロンプトを表示
    st.markdown('<div class="ai-box" style="margin-top: 20px;">', unsafe_allow_html=True)
    st.subheader("📋 【商品説明文用】AI用コピーテキストの作成完了")
    st.success("✨ 下の枠内のテキストをすべてコピーして、ChatGPTやGeminiのチャット欄に貼り付けてください。")
    
    # コピーしやすいようにテキストエリアで表示
    st.text_area("以下の文章を丸ごとコピーしてください：", value=final_desc_prompt, height=500, key="copy_area_desc")
    st.markdown('</div>', unsafe_allow_html=True)
