...
# =================================================================
if generate_btn:
    with st.spinner("📝 タイトル用プロンプトを作成中..."):
        items_summary = ""
        for _, row in candidate_items.iterrows():
            items_summary += f"・{row['商品名']} (購入者数: {row['_buy_num']}人)\n"
            
        final_title_prompt = f"""あなたはハンドメイドマーケット（Creemaやminne）の市場リサーチとコピーライティングのプロフェッショナルです。
以下の【分析対象：売れている商品のタイトル一覧】と【出品する作品の情報】を元に、売れる商品タイトルを考えてください。
...
### 2. 新作の作品紹介文の提案（3選）
分析した人気商品の紹介文の構成をベースに、以下の要素をストーリーの中に深く、自然に組み込んで、そのまま出品に使える紹介文を【3つの異なる雰囲気（例：上品で丁寧なトーン、天然石の物語を伝えるエモーショナルなトーン、日常使いをイメージさせるカジュアルなトーン）】で提案してください。

⚠️【紹介文に必ず入れる必須要素】：
1. **購入検討者が抱えている「悩みや願望」の深い掘り下げ**
   （ただの肩こり・冷え性という表面的な悩みではなく、「夕方になると重だるくて、家族や周りに優しくできない」「健康グッズはオバサンっぽくて、着けているのを見られたくない」「不器用だから毎朝ブレスレットを着けるだけで爪が痛む・遅刻しそうになる」など、日常のリアルな葛藤やイライラに共感する文章にしてください）

2. **この商品を手に入れることで、お客様が到達する「最高の未来（ベネフィットとその結末）」**
   （「見るたびに癒される」「自信を持って身につけられる」「周りから褒められる」という一段階目の未来の【その結果、人生がどう変わるか】まで想像力を膨らませて書いてください。例：手元を見るたびに呼吸が深くなり心に余裕が生まれる、褒められたことでお出かけがもっと楽しみになる、身体がラクになることで一日中笑顔でいられて自分を好きになれる、など）

3. **お客様が実際に商品を使用している姿・着用シーンの具体的な描写**
   （休日のカフェでコーヒーカップを持つ手元がきらりと光る瞬間や、オフィスでパソコンのキーボードを打つたびに美しい天然石が目に入り気分が上がる様子など、お客様が「自分がこれを身につけたらこんな素敵な毎日になるんだ」と、ありありと映像で想像できるようなシチュエーションを必ず入れてください）

4. **人気商品の紹介文から学んだ「売れる構成とキーワード」**

### 3. 出品時のワンポイントアドバイス
* 発送方法の書き方や、紹介文のカスタマイズなど、この紹介文を使って出品する際のアドバイスを1つ教えてください。

### 4. 検索対策キーワード一覧（文末に配置）
* この商品をCreemaやminneで検索してもらうために、今回の分析結果と作品情報（素材・お悩み・ターゲット）を掛け合わせた、ハッシュタグや検索窓に入力されやすいキーワードを20個以上、スペース区切りでずらりと一覧化してください。
"""
